...
type SavedScreenPlanItem = ScreenPlanItem & {
  imageSrc?: string
}

type DevelopmentScreenDraft = {
  id: string
  name: string
  routePath: string
  relativePath: string
  status: 'pending' | 'working' | 'generated' | 'failed'
  message?: string
}

type DevelopmentDraft = {
  project?: BaseProject
  status?: 'working' | 'completed' | 'partial' | 'failed'
  statusText?: string
  screens?: DevelopmentScreenDraft[]
  completedAt?: string
  updatedAt?: string
}
...
type DraftData = {
  ...
  screens?: SavedScreenPlanItem[]
  development?: DevelopmentDraft | null
}
...
type ScreenFileStatus = {
  exists: boolean
  relativePath: string
  routePath: string
}

type BaseProjectStatus = {
  exists: boolean
  relativePath: string
}
...
let currentDatabaseDesign: DatabaseDesign | undefined
let currentScreens: ScreenPlanItem[] = []
let currentDraftFilename: string | undefined
let codexDebugTimer: number | undefined
let screenFilePollingTimer: number | undefined
let activeDevelopmentProjectId: string | undefined
let currentDevelopmentDraft: DevelopmentDraft | undefined
...
const showSection = (element: HTMLElement) => {
  element.classList.remove('hidden')
  element.classList.add('revealed')
}
...
const setIntroError = (message: string) => {
  introError.textContent = message
  introError.classList.toggle('hidden', !message)
}

const renderRecentDrafts = (drafts: DraftSummary[]) => {
...
const createWorkflow = async (projectText: string): Promise<WorkflowStep[]> => {
...
const normalizeScreen = (value: string) =>
  value
    .trim()
    .replace(/画面で$/, '')
    .replace(/画面$/, '')
...
const getWorkflowScreenOptions = (steps: WorkflowStep[] = []) =>
  Array.from(
    new Set(
      [...defaultScreenOptions, ...steps.map((step) => step.screen)]
        .filter(Boolean)
        .map(normalizeScreen),
    ),
  )

const createWorkflowRow = (
  step: WorkflowStep = { actor: '', screen: '', action: '' },
  screenChoices = defaultScreenOptions,
) => {
  ...
  const screenOptions = Array.from(
    new Set([...screenChoices, step.screen].filter(Boolean).map(normalizeScreen)),
  )
  ...
  deleteButton.addEventListener('click', () => {
    if (workflowList.children.length <= 1) return
    item.remove()
  })
...
const collectWorkflowSteps = () => {
  return Array.from(workflowList.querySelectorAll('li'))
    .map((row) => ({
      actor:
        row.querySelector<HTMLSelectElement>('select[aria-label="業務の主体"]')?.value.trim() ??
        '',
      screen: normalizeScreen(
        row.querySelector<HTMLSelectElement>('select[aria-label="業務の画面"]')?.value ?? '',
      ),
      action:
        row.querySelector<HTMLInputElement>('input[aria-label="業務の動作"]')?.value.trim() ??
        '',
    }))
    .filter((step) => step.actor && step.action)
}

const setDatabaseError = (message: string) => {
  databaseError.textContent = message
  databaseError.classList.toggle('hidden', !message)
}

const setDatabaseLoading = (loading: boolean) => {
  createWorkflowButton.disabled = loading
  createWorkflowButton.textContent = loading ? '設計中' : '作成'
}

const createDatabaseDesign = async (
  workflowSteps: WorkflowStep[],
...
const setDatabaseTab = (tab: 'er' | 'tables') => {
  const isEr = tab === 'er'
...
const renderDatabaseDesign = (databaseDesign: DatabaseDesign) => {
...
const createScreenPlan = async (
  ...
  const data = (await response.json().catch(() => ({}))) as {
    error?: string
    screenPlan?: { screens?: ScreenPlanItem[] }
  }
...
const createScreenImage = async (
  screen: ScreenPlanItem,
...
const createBaseProject = async (): Promise<BaseProject> => {
...
const implementBaseProjectScreen = async (
  projectId: string,
  screen: SavedScreenPlanItem,
): Promise<ImplementedScreen> => {
  const response = await fetch(
    `/api/base-projects/${encodeURIComponent(projectId)}/screens/${encodeURIComponent(screen.id)}`,
    {
      method: 'POST',
      headers: {
        'content-type': 'application/json',
...
const implementBaseProjectScreens = async (
  projectId: string,
  screens: SavedScreenPlanItem[],
): Promise<ImplementedScreen[]> => {
  const response = await fetch(
    `/api/base-projects/${encodeURIComponent(projectId)}/screens`,
    {
      method: 'POST',
      headers: {
        'content-type': 'application/json',
...
const fetchScreenFileStatus = async (
  projectId: string,
  screenId: string,
): Promise<ScreenFileStatus | undefined> => {
  const response = await fetch(
    `/api/base-projects/${encodeURIComponent(projectId)}/screens/${encodeURIComponent(screenId)}/file-status`,
  )
  const data = (await response.json().catch(() => ({}))) as {
    status?: ScreenFileStatus
  }

  return response.ok ? data.status : undefined
}

const fetchBaseProjectStatus = async (
  projectId: string,
): Promise<BaseProjectStatus | undefined> => {
  const response = await fetch(
    `/api/base-projects/${encodeURIComponent(projectId)}/status`,
  )
  const data = (await response.json().catch(() => ({}))) as {
    error?: string
    status?: BaseProjectStatus
  }
...
const checkCodexServerStatus = async (): Promise<CodexServerStatus> => {
  const response = await fetch('/api/codex-server-status')
  const data = (await response.json().catch(() => ({}))) as Partial<CodexServerStatus>
  return {
...
const startCodexDebugPolling = () => {
  if (codexDebugTimer !== undefined) return
...
const stopScreenFilePolling = () => {
  if (screenFilePollingTimer === undefined) return
  window.clearInterval(screenFilePollingTimer)
  screenFilePollingTimer = undefined
}

const screenRouteId = (screen: { id?: string; name?: string }) => {
...
const screenRouteTarget = (screen: { id?: string; name?: string }) => {
...
const screenRouteTargetText = (screen: { id?: string; name?: string }) => {
  const target = screenRouteTarget(screen)
  return `生成先: ${target.routePath} / ${target.filePath}`
}

const createDevelopmentScreenDrafts = (
  screens: SavedScreenPlanItem[],
): DevelopmentScreenDraft[] =>
  screens.map((screen) => {
    const target = screenRouteTarget(screen)
    return {
      id: screen.id,
...
const updateDevelopmentDraft = (
  updates: Partial<DevelopmentDraft>,
  persist = true,
) => {
...
const updateDevelopmentScreenDraft = (
  screen: { id?: string; name?: string },
  updates: Partial<DevelopmentScreenDraft>,
  persist = true,
) => {
  if (!currentDevelopmentDraft?.screens || !screen.id) return

  let changed = false
  currentDevelopmentDraft = {
    ...currentDevelopmentDraft,
    screens: currentDevelopmentDraft.screens.map((item) => {
      if (item.id !== screen.id) return item
...
const applyDevelopmentScreenDraft = (screen: DevelopmentScreenDraft) => {
...
const renderBaseProject = (project: BaseProject) => {
...
const findDevelopmentScreenItem = (screen: { id?: string; name?: string }) => {
  const escapedId = screen.id ? CSS.escape(screen.id) : ''
  const escapedName = screen.name ? CSS.escape(screen.name) : ''
  return developmentScreens.querySelector<HTMLElement>(
    [
...
const setDevelopmentScreenWorking = (
  screen: { id?: string; name?: string },
) => {
  const item = findDevelopmentScreenItem(screen)
  const status = item?.querySelector<HTMLButtonElement>('.development-screen-status')
  const summary = item?.querySelector('p')
  if (!item || !status || !summary) return
...
const setDevelopmentScreenDone = (
  sourceScreen: SavedScreenPlanItem,
  implementedScreen: ImplementedScreen,
) => {
...
const setDevelopmentScreenGeneratedFromFile = (
  screen: SavedScreenPlanItem,
  fileStatus: ScreenFileStatus,
) => {
...
const setDevelopmentScreenFileCreated = (
  screen: SavedScreenPlanItem,
  fileStatus: ScreenFileStatus,
) => {
...
const setDevelopmentScreenFailed = (
  screen: { id?: string; name?: string },
  message: string,
) => {
  const item = findDevelopmentScreenItem(screen)
...
const renderDevelopmentScreenStatuses = (screens: SavedScreenPlanItem[]) => {
...
const setDevelopmentScreensFailed = () => {
  developmentScreens
    .querySelectorAll<HTMLElement>('.development-screen-item')
    .forEach((item) => {
...
const retryDevelopmentScreen = async (screen: { id?: string; name?: string }) => {
  if (!activeDevelopmentProjectId || !screen.id || !screen.name) return
  ...
  try {
    const implementedScreen = await implementBaseProjectScreen(
      activeDevelopmentProjectId,
      savedScreen,
    )
    ...
    if (status?.exists) {
      ...
      setDevelopmentScreenFailed(
        savedScreen,
        error instanceof Error ? error.message : '画面を実装できませんでした',
      )
    }
  } finally {
    stopScreenFilePolling()
...
const markScreensByGeneratedFiles = async (
  projectId: string,
  screens: SavedScreenPlanItem[],
...
const renderScreenPlan = (screens: ScreenPlanItem[]) => {
...
const renderSavedScreenImages = (screens: SavedScreenPlanItem[]) => {
  screens.forEach((screen) => {
    if (!screen.imageSrc) return
...
const collectScreensForDraft = (): SavedScreenPlanItem[] =>
  currentScreens.map((screen) => {
    const image = screensList.querySelector<HTMLImageElement>(
      `[data-screen-id="${screen.id}"] img`,
    )
    return {
      ...screen,
...
const restoreDevelopmentDraft = async (
  development: DevelopmentDraft | null | undefined,
) => {
  ...
  const statusText =
    currentDevelopmentDraft?.statusText ??
...
const fillSingleScreenImage = async (screen: ScreenPlanItem) => {
  const card = screensList.querySelector(`[data-screen-id="${screen.id}"]`)
  const loading = card?.querySelector('.screen-image-loading')
  const image = card?.querySelector<HTMLImageElement>('img')
  if (!card || !loading || !image) return
...
const fillScreenImages = async (screens: ScreenPlanItem[]) => {
  let nextIndex = 0
...
databaseOkButton.addEventListener('click', async () => {
  ...
  try {
    const screens = await createScreenPlan(
      workflowSteps,
      currentDatabaseDesign?.tables ?? [],
    )
...
screensOkButton.addEventListener('click', () => {
  ...
  void (async () => {
    ...
    try {
      const implementedScreens = await implementBaseProjectScreens(
        project.id,
        screensForDevelopment,
      )
      stopScreenFilePolling()
      const implementedById = new Map(
        implementedScreens.map((screen) => [screen.id, screen]),
      )
      ...
      stopScreenFilePolling()
      const result = await markScreensByGeneratedFiles(
        project.id,
        screensForDevelopment,
        error instanceof Error ? error.message : '画面を実装できませんでした',
      )
    ...
    if (
      !developmentStatus.textContent.startsWith('完了:') &&
